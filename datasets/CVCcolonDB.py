...
class ColonDataset(Dataset):
    def __init__(self, root_dir, image_size=512):
        """
        Creates the CVC-ColonDB dataset.
        ...
        self.image_paths = glob.glob(root_dir+'/images/*.png')
        self.mask_paths = glob.glob(root_dir+'/masks/*.png')
...
if __name__ == '__main__':
  root_dir = "/content/drive/MyDrive/UFF/CVC-ColonDB" 
  colon_dataset = ColonDataset(root_dir=root_dir)
  print(f'image size : {colon_dataset.__getitem__(0)[0].size()}')
  print(f'mask size : {colon_dataset.__getitem__(0)[0].size()}')  